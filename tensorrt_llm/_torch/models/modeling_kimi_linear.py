...
Caching
-------
KDA states live on the recurrent-state side of a
``KimiKDAHybridCacheManagerV2`` (wired in ``pyexecutor/_util.py``): per layer,
a short-conv slot of
...
the V2 KDA cache manager (per-request KDA state snapshots at
``mamba_state_config.periodic_snapshot_interval`` boundaries,
FORCE_CHUNK context chunking).
...
class KimiLinearForCausalLM(SpecDecOneEngineForCausalLM[KimiLinearModel, Any]):
    ...
    def get_model_defaults(cls, llm_args) -> dict:
        # - KimiKDAHybridCacheManagerV2 owns the KDA replay state required by
        #   SA and DFlash speculative decoding.
        # - enable_block_reuse defaults off: V2 recurrent-state reuse remains
        #   an explicit opt-in that requires a snapshot policy.
    ...
    def get_preferred_transceiver_runtime(
        ...
        """Use the Python transceiver for KDA and MLA state transfers."""
        return "PYTHON"
